...
# 1. Patch Classifier
PATCH_MODEL_PATH = "checkpoints_cls/resnet18_focal_best.pth"
...
# [新增] 全局排队锁：同一时间只允许一个人使用 GPU
gpu_lock = threading.Lock()

# Maps
LABEL_MAP = {0: 0, 1: 1, 2: 2, 3: 3, 6: 4, 7: 5}
INV_MAP   = {v: k for k, v in LABEL_MAP.items()}
CLASS_NAMES = {0: "Others", 1: "T", 2: "V", 3: "I", 6: "Stela", 7: "Fibrosis"}
SLOPE_NAMES = {0: "正常 (Straight)", 1: "倾斜 (Inclined)"}
LAYER_NAMES = {0: "真皮上部 (Upper)", 1: "真皮中部 (Mid)", 2: "真皮下部 (Lower)", 3: "皮下脂肪 (Subcutis)"}
COLORS = {0: (160, 160, 160), 1: (0, 0, 255), 2: (0, 255, 0), 3: (255, 0, 0), 6: (0, 255, 255), 7: (255, 0, 255)}
...
try:
    ...
    print("[OK] Patch Classifier loaded.")
...
def process_image(img_path, filename):
    ...
    for m in masks:
        ...
        nms_score = score.item() + (1.0 if final_id != 0 else 0) # Structure bonus
    ...
    # 4. Draw
    overlay = bgr.copy()
    stats = {1:0, 2:0, 3:0, 6:0, 7:0, 0:0, 'total_follicles':0}
    for det in final_detections:
        x, y, w, h = det['bbox']
        cid = det['class_id']
        stats[cid] += 1
        if cid in [1, 2, 3, 6]: stats['total_follicles'] += 1
...
<head>
    ...
    <style>
        ...
        .status-idle { background-color: #d4edda; color: #155724; border: 1px solid #c3e6cb; } /* Green */
        .status-busy { background-color: #fff3cd; color: #856404; border: 1px solid #ffeeba; } /* Yellow */
